...
const ResponseError_1 = require("../exception/ResponseError");
class Command {
    constructor() {
        this.requestParser = Command.getParsers().request;
        this.responseParser = Command.getParsers().response;
        this.cmd = "";
        this.options = {};
        this.multiOpts = [];
        this.flags = [];
     ...
     */
    setError(raw) {
        this.error = Command.parse({ raw })[0];
        return this;
    }
    ...
    getError() {
        if (!this.hasError())
            return null;
        return new ResponseError_1.ResponseError(this.error, this.stack);
    }
     ...
     */
    static parseSnapshotCreate({ raw }) {
        const version = raw.match(/version=(\d+)/);
        if (!version)
            throw new Error("unable to detect snapshot version");
        switch (version[1]) {
            case "2":
                return (() => {
     ...
     */
    static buildSnapshotDeploy(data, cmd, { version }, snapshotVersion = "0") {
     ...
     */
    static minVersion(minimum, version) {
    ...
    static build(command) {
        ...
        if (command.hasFlags())
            cmd += ` ${command.buildFlags()}`;
        if (command.hasOptions())
            cmd += ` ${command.buildOptions()}`;
        return cmd;
    }
    /**
     ...
     * @return the parsed String which is readable by the TeamSpeak Query
     */
    static escapeKeyValue(key, value) {
     ...
     */
    static getKeyValue(str) {
        const index = str.indexOf("=");
        if (index === -1)
            return { key: str, value: undefined };
        const value = str.substring(index + 1);
        return { key: Command.toCamelCase(str.substring(0, index)),
    ...
    static toSnakeCase(str) {
        return str.split("").map(char => {
            ...
            if (char !== lower)
                return `${Command.SNAKE_CASE_IDENTIFIER}${lower}`;
            return char;
        }).join("");
    }
}
exports.Command = Command;
Command.SNAKE_CASE_IDENTIFIER = "_";
(function (Command) {
    Command.Identifier = {
